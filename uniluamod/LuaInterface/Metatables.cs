namespace UniLuaInterface
{
    ...
    class MetaFunctions
    {
        ...
        internal CSharpFunctionDelegate gcFunction, indexFunction, newindexFunction,
            baseIndexFunction, classIndexFunction, classNewindexFunction,
            execDelegateFunction, callConstructorFunction, toStringFunction, test;
        ...
        private int testDelegate(ILuaState luaState)
        {
         ...
         */
        private int collectObject(ILuaState luaState)
        {
         ...
         */
        private int toString(ILuaState luaState)
        {
            object obj = translator.getRawNetObject(luaState, 1);
            if (obj != null)
            {
        ...
        public static void dumpStack(ObjectTranslator translator, ILuaState luaState)
        {
            ...
            for (int i = 1; i <= depth; i++)
            {
                ...
                string typestr = (type == LuaType.LUA_TTABLE) ? "table" : LuaDLL.lua_typename(luaState, type);

                string strrep = LuaDLL.lua_tostring(luaState, i);
                if (type == LuaType.LUA_TUSERDATA || type == LuaType.LUA_TLIGHTUSERDATA)
                {
         ...
         */
        private int getMethod(ILuaState luaState)
        {
            object obj = translator.getRawNetObject(luaState, 1);
            if (obj == null)
            {
         ...
         */
        private int getBaseMethod(ILuaState luaState)
        {
            object obj = translator.getRawNetObject(luaState, 1);
            if (obj == null)
            {
            ...
            if (LuaDLL.lua_type(luaState, -1) == LuaType.LUA_TNIL)
            {
                LuaDLL.lua_settop(luaState, -2);
         ...
         */
        private int setFieldOrProperty(ILuaState luaState)
        {
        ...
        private bool trySetMember(ILuaState luaState, IReflect targetType, object target, BindingFlags bindingType, out string detailMessage)
        {
            ...
            if (LuaDLL.lua_type(luaState, 2) != LuaType.LUA_TSTRING)
            {
         ...
         */
        private int setMember(ILuaState luaState, IReflect targetType, object target, BindingFlags bindingType)
        {
            string detail;
        ...
        void ThrowError(ILuaState luaState, Exception e)
        {
         ...
         */
        private int getClassMethod(ILuaState luaState)
        {
            IReflect klass;
            object obj = translator.getRawNetObject(luaState, 1);
         ...
         */
        private int setClassFieldOrProperty(ILuaState luaState)
        {
            IReflect target;
            object obj = translator.getRawNetObject(luaState, 1);
            if (obj == null || !(obj is IReflect))
        ...
        /// <returns></returns>
        private bool _IsTypeCorrect(ILuaState luaState, int currentLuaParam, ParameterInfo currentNetParam, out ExtractValue extractValue)
        {
            try
            {
        ...
        private bool _IsParamsArray(ILuaState luaState, int currentLuaParam, ParameterInfo currentNetParam, out ExtractValue extractValue)
        {
            extractValue = null;
