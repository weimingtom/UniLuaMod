...
l_tforloop:
		...
		private void V_GetTable( StkId t, StkId key, StkId val )
		{
			for( int loop=0; loop<MAXTAGLOOP; ++loop ) {
				StkId tmObj;
				if(t.V.TtIsTable()) {
					LuaTable tbl = t.V.HValue();
					StkId res = tbl.Get( ref key.V );
					if( !res.V.TtIsNil() ) {
		...
		private void V_SetTable(StkId t, StkId key, StkId val)
		{
			for( int loop=0; loop<MAXTAGLOOP; ++loop ) {
				...
				if(t.V.TtIsTable()) {
					LuaTable tbl = t.V.HValue();
					StkId oldval = tbl.Get(ref key.V);
					if(!oldval.V.TtIsNil()) {
		...
		{
			LuaLClosureValue ncl = new LuaLClosureValue( p );
...
calltm:
		...
		private void V_Concat( int total )
		{
			...
			do
			{
				StkId top = Top;
				int n = 2;
				StkId lhs = Stack[top.Index - 2];
				StkId rhs = Stack[top.Index - 1];
		...
		{
			StkId tmObj = T_GetTMByObj(ref p1.V, tm);
			if(tmObj.V.TtIsNil())
		...
		{
			TValue nb = new TValue();
			TValue nc = new TValue();
			if(V_ToNumber(rb, ref nb) && V_ToNumber(rc, ref nc))
			{
				double res = O_Arith( TMS2OP(op), nb.NValue, nc.NValue );
				ra.V.SetNValue( res );
			}
		...
		private bool V_LessThan( StkId lhs, StkId rhs )
		{
			...
			bool error;
			bool res = CallOrderTM( lhs, rhs, TMS.TM_LT, out error );
			if( error )
			{
		...
		private bool V_LessEqual( StkId lhs, StkId rhs )
		{
			...
			// first try `le'
			bool error;
			bool res = CallOrderTM( lhs, rhs, TMS.TM_LE, out error );
			if( !error )
				return res;
		...
		private void V_FinishOp()
		{
			...
			switch( op )
			{
				...
				{
					StkId tmp = Stack[stackBase + i.GETARG_A()];
					Top = Stack[Top.Index-1];
				...
				case OpCode.OP_CONCAT:
				{
					...
					CallInfo ci = BaseCI[ciIndex];
					StkId tmp2 = Stack[ci.BaseIndex + i.GETARG_A()];
