...
namespace UniLua
{
	...
	internal class LuaFFILib
	{
		...
		private static int FFI_ParseSignature( ILuaState lua )
		{
			...
			FuncSignature result = FuncSignatureParser.Parse( lua, signature );
		...
		private static int FFI_GetConstructor( ILuaState lua )
		{
			...
			FFIConstructorInfo ffiMethod = new FFIConstructorInfo(cinfo);
		...
		private static int GetMethodAux( ILuaState lua, BindingFlags flags )
		{
			...
			MethodInfo minfo = t.GetMethod( mname,
				flags,
				null,
			...
			{
				FFIMethodInfo ffiMethod = new FFIMethodInfo(minfo);
				lua.PushLightUserData( ffiMethod );
				return 1;
			}
		}
		...
		{
			FFIMethodBase ffiMethod = (FFIMethodBase)lua.ToUserData(1);
			if (ffiMethod != null)
			{
				try
				{
		...
		{
			FieldInfo finfo = (FieldInfo)lua.ToUserData(1);
			object inst = lua.ToUserData(2);
			Type returnType = (Type)lua.ToUserData(3);
			object value = finfo.GetValue( inst );
		...
		private static int FFI_SetFieldValue( ILuaState lua )
		{
			...
			object value = LuaStackUtil.ToRawValue(lua, 3, t);
		...
		{
			PropertyInfo pinfo = (PropertyInfo)lua.ToUserData(1);
			object inst = lua.ToUserData(2);
			Type returnType = (Type)lua.ToUserData(3);
			object value = pinfo.GetValue( inst, null );
			LuaStackUtil.PushRawValue( lua, value, returnType );
			return 1;
		}
...
//////////////////////////////////////////////////////////////////////
		...
		private static Type FindTypeInAllAssemblies(string typename)
		{
			...
			{
				Type t = AssemblyList[i].GetType( typename );
				if( t != null )
				{
					if(result == null) {
						result = t;
		...
		{
			Type result = FindTypeInAllAssemblies( typename );
			if( result != null )
				return result;

			for( int i=0; i<UsingList.Count; ++i )
			{
				string fullname = UsingList[i] + "." + typename;
...
//////////////////////////////////////////////////////////////////////
		...
		class FuncSignatureParser
		{
				...
				ILuaState lua, string signature )
			{
				...
				FuncSignatureParser parser = new FuncSignatureParser();
			...
			private void ReturnType()
			{
				...
				{
					NameToken t = Lexer.Token as NameToken;
					if( t != null )
					{
