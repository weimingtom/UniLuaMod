...
{	
	...
	public partial class LuaState
	{
				...
				switch( c )
				{
					...
					case 'u':
					{
						...
						if(func.V.ClIsLuaClosure()) {
							...
							LuaCsClosureValue ccl = func.V.ClCsValue();
					...
					case 'n':
					{
						CallInfo prevCI = BaseCI[ci.Index-1];
						if( ci != null
							&& ((ci.CallStatus & CallStatus.CIST_TAIL) == 0)
							&& prevCI.IsLua )
						{
		...
		private void AddInfo( string msg )
		{
			...
			if( CI.IsLua )
			{
				int line = GetCurrentLine(CI);
				string src = GetCurrentLuaFunc(CI).Proto.Source;
				if( src == null )
					src = "?";
		...
		private void KName( LuaProto proto, int pc, int c, out string name )
		{
			if( Instruction.ISK(c) ) { // is `c' a constant
				StkId val = proto.K[Instruction.INDEXK(c)];
		...
		private void G_TypeError( StkId o, string op )
		{
			...
			if( ci.IsLua )
			{
				...
				{
					LuaLClosureValue lcl = Stack[ci.FuncIndex].V.ClLValue();
