...
namespace UniLua
{
	...
	public class Parser
	{
			...
			ILuaState lua, ILoadInfo loadinfo, string name )
		{
			...
			FuncState topFuncState = new FuncState();
		...
		{
			LabelDesc gt = PendingGotos[g];
			BlockCnt block = CurFunc.Block;
			for( int i=block.FirstLabel; i<ActiveLabels.Count; ++i )
			{
				LabelDesc label = ActiveLabels[i];
		...
		{
			LabelDesc desc = NewLebelEntry( "break", 0, CurFunc.Pc );
		...
		private void UndefGoto( LabelDesc gt )
		{
			...
			string msg = string.Format( template, gt.Name, gt.Line );
			SemanticError( msg );
		}
		...
		private void LabelStat( string label, int line )
		{
			...
			LabelDesc desc = NewLebelEntry( label, line, fs.Pc );
			ActiveLabels.Add( desc );
			SkipNoOpStat();
		...
		private void RepeatStat( int line )
		{
			...
			BlockCnt blockLoop  = new BlockCnt();
			BlockCnt blockScope = new BlockCnt();
		...
		private void LocalFunc()
		{
			...
			FuncState fs = CurFunc;
			VarDesc v = NewLocalVar(CheckName());
			ActVars.Add(v);
		...
		private void ExprStat()
		{
			...
			else
			{
				...
				InstructionPtr pi = CurFunc.GetCode( v.Exp );
		...
		private void CheckConflict( LHSAssign lh, ExpDesc v )
		{
			...
			if( conflict )
			{
				...
				OpCode op = (v.Kind == ExpKind.VLOCAL) ? OpCode.OP_MOVE : OpCode.OP_GETUPVAL;
		...
		private void Constructor( ExpDesc t )
		{
			...
			ConstructorControl cc = new ConstructorControl();
			cc.ExpTable = t;
			...
			Instruction ins = fs.Proto.Code[pc];
		...
		private void FuncArgs( ExpDesc e, int line )
		{
			...
			switch( Lexer.Token.TokenType )
			{
				...
				// funcargs -> STRING
				case (int)TK.STRING: {
					StringToken st = Lexer.Token as StringToken;
		...
		private void SimpleExp( ExpDesc e )
		{
			...
			{
				case (int)TK.NUMBER: {
					NumberToken nt = t as NumberToken;
				...
				case (int)TK.STRING: {
					StringToken st = t as StringToken;
