...
namespace UniLua
{
	...
	public class LuaTable {
		...
		public StkId Get(ref TValue key)
		{
			...
			for(HNode node = GetHashNode(h); node != null; node = node.Next) {
				if(node.Key.V == key) {
					{ return node.Val; }
				}
			}
		...
		public StkId GetInt(int key)
		{
			...
			TValue k = new TValue();
			k.SetNValue(key);
			for(HNode node = GetHashNode(ref k); node != null; node = node.Next) {
		...
		*/
		...
		{
			//uint n = (uint)(hashcode); //FIXME:System.OverflowException
			uint n = (uint)(hashcode & (0xffffffff >> 1)); 
		...
		*/
		...
		private void DumpParts()
		{
			...
			for(int i=0; i<HashPart.Length; ++i) {
				HNode n = HashPart[i];
				int next = (n.Next == null) ? -1 : n.Next.Index;
