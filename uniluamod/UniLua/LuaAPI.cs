...
namespace UniLua
{
	...
	public partial class LuaState : ILuaState
	{
		...
		private static void F_Load(ref LoadParameter param)
		{
			...
			LuaLClosureValue cl = new LuaLClosureValue( proto );
		...
		{
			LoadParameter param  = new LoadParameter(this, loadinfo, name, mode);
			ThreadStatus status = D_PCall( DG_F_Load, ref param, Top.Index, ErrFunc );

			if( status == ThreadStatus.LUA_OK ) {
				StkId below = Stack[Top.Index-1];
				Utl.Assert(below.V.TtIsFunction() && below.V.ClIsLuaClosure());
				LuaLClosureValue cl = below.V.ClLValue();
		...
		DumpStatus ILuaAPI.Dump( LuaWriter writeFunc )
		{
			...
			LuaLClosureValue o = below.V.ClLValue();
			if(o == null)
				return DumpStatus.ERROR;
		...
		ThreadStatus ILuaAPI.Resume( ILuaState from, int numArgs )
		{
			...
			else // yield or regular error
			{
				while( status != ThreadStatus.LUA_OK && status != ThreadStatus.LUA_YIELD ) // error?
				{
					...
					{
						UnrollParam unrollParam = new UnrollParam();
						unrollParam.L = this;
		...
		bool ILuaAPI.CheckStack(int size)
		{
			...
			// need to grow stack
				...
				if(inuse > LuaConf.LUAI_MAXSTACK - size)
					...
					GrowStackParam param = new GrowStackParam();
					param.L = this;
					param.size = size;
		...
		private string AuxUpvalue(StkId addr, int n, out StkId val)
		{
			...
			if(addr.V.ClIsLuaClosure()) {
				LuaLClosureValue f = addr.V.ClLValue();
				LuaProto p = f.Proto;
				if(!(1 <= n && n <= p.Upvalues.Count))
					return null;
				val = f.Upvals[n-1].V;
				string name = p.Upvalues[n-1].Name;
				return (name == null) ? "" : name;
			}
			else if(addr.V.ClIsCsClosure()) {
				LuaCsClosureValue f = addr.V.ClCsValue();
		...
		string ILuaAPI.GetUpvalue( int funcIndex, int n )
		{
			...
			StkId val;
			string name = AuxUpvalue(addr, n, out val);
			if(name == null)
				return null;
		...
		void ILuaAPI.SetTable( int index )
		{
			...
			StkId key = Stack[Top.Index - 2];
			StkId val = Stack[Top.Index - 1];
		...
		bool ILuaAPI.GetMetaTable( int index )
		{
			...
			switch( addr.V.Tt )
			{
				...
				case (int)LuaType.LUA_TUSERDATA:
				{
					LuaUserDataValue ud = addr.V.RawUValue();
					mt = ud.MetaTable;
					break;
				}
				default:
		...
		private bool Index2Addr( int index, out StkId addr )
		{
			...
			if( index > 0 )
			{
				int addrIndex = ci.FuncIndex + index;
			...
			else
			{
				...
				LuaCsClosureValue clcs = func.V.ClCsValue();
