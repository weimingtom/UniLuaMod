	...
	public static class Coder
	{
			...
			ExpDesc e1, ExpDesc e2, int line )
		{
			switch( op )
			{
				...
				case BinOpr.CONCAT: {
					Exp2Val( fs, e2 );
					InstructionPtr pe2 = fs.GetCode( e2 );
		...
		public static void PatchClose( FuncState fs, int list, int level )
		{
			...
			{
				int next = GetJump( fs, list );
				InstructionPtr pi = new InstructionPtr( fs.Proto.Code, list );;
		...
		public static void CodeNil( FuncState fs, int from, int n )
		{
			...
			{
				InstructionPtr previous = new InstructionPtr( fs.Proto.Code, fs.Pc-1 );
