...
namespace UniLua
{
	...
	internal class DumpState
	{
		...
		{
			byte[] bytes = new byte[LUAC_HEADERSIZE];
			int i = 0;

			for(int j=0; j<LuaConf.LUA_SIGNATURE.Length; ++j)
			...
			for(int j=0; j<LUAC_TAIL.Length; ++j)
		...
		{
			DumpVector( proto.K, delegate(StkId k) {
				int t = k.V.Tt;
				DumpByte( (byte)t );
				switch( t )
				{
			...
			DumpVector( proto.P, delegate (LuaProto p) {
				DumpFunction( p );
			});
		}
		...
		{
			DumpVector( proto.Upvalues, delegate (UpvalDesc upval) {
